           ...
           if doneOr
              ...
              elseif POP_STRUC.POPULATION(whichInd).JobID ~= 0.02   % no optimization
                 ...
                 if Error == 0
                    ...
                    [lat,candidate,vol,atyplist,numIons]=reduce_Surface_201(lattice,coordinate,whichInd);
                    ...
                    if Step < length ([ORG_STRUC.abinitioCode]) 
                       ...
                       [lat,candidate,numIons,typesAList,chanAList] = makeSurface(lat,surcoor,surnumIons,bulklat,bulkcoor,atyp,ntyp,ORG_STRUC.vacuumSize(Step));
              ...
              if POP_STRUC.POPULATION(whichInd).Error > ORG_STRUC.maxErrors
               ...
               elseif POP_STRUC.POPULATION(whichInd).Step > length ([ORG_STRUC.abinitioCode]) %Note Step is differnt !!!!!
                  POP_STRUC.POPULATION(whichInd).Done = 1;
                  POP_STRUC.POPULATION(whichInd).ToDo = 0;
                  ...
                  numIons = sum(POP_STRUC.POPULATION(whichInd).Surface_numIons);

                  [Ni, V, dist_matrix, typ_i, typ_j, ho, ht] = makeMatrices_2D(LATTICE, COORDINATES, numIons, 1);
                  ...
                  WriteIndividualOutput_201(whichInd);
                  POP_STRUC.POPULATION(whichInd).Folder=0;
              end
