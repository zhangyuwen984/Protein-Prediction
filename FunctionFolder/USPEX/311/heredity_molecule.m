function [numMols, numBlocks, offspring, potentialLattice, fracFrac,dimension,offset,MolTable] = heredity_molecule(par_one,par_two)
...
numIons1 = POOL_STRUC.POPULATION(par_one).numMols;
numIons2 = POOL_STRUC.POPULATION(par_two).numMols;

parent1 = zeros(sum(numIons1),3);
parent2 = zeros(sum(numIons2),3);

for ind = 1 : sum(numIons1)
    parent1(ind,:) = POOL_STRUC.POPULATION(par_one).MOLECULES(ind).MOLCENTER;
    order1(ind)    = POOL_STRUC.POPULATION(par_one).MOLECULES(ind).order;
end

for ind = 1 : sum(numIons2)
    parent2(ind,:) = POOL_STRUC.POPULATION(par_two).MOLECULES(ind).MOLCENTER;
    order2(ind)    = POOL_STRUC.POPULATION(par_two).MOLECULES(ind).order;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 maxBlocks = POOL_STRUC.POPULATION(par_one).numBlocks + POOL_STRUC.POPULATION(par_two).numBlocks;
...
if ~ORG_STRUC.constLattice
    ...
    if (size(ORG_STRUC.numIons,1) == 2) &  sameComp
     ...
     % works only for 2 types of atoms!
         ...
         v1 = det(POOL_STRUC.POPULATION(ch([i+1],N_T+2)).LATTICE);
         v2 = det(POOL_STRUC.POPULATION(ch([i],N_T+2)).LATTICE);
