function [freq, eigvector] = calcSoftModes(N_val, val1, numIons, lat, coords)
...
if ORG_STRUC.dimension==0 %cluster 
 nL = 0;
end
...
elseif ORG_STRUC.dimension==2 %surface
  for i = 1 : N_i    
   for j = i : N_i
    for k1 = -1 : 1
     for k2 = -1 : 1
   ...
   for m1 = 1 : 2*nL+1
    for m2 = 1 : 2*nL+1
     for m3 = 1 : 2*nL+1
      ...
      if ORG_STRUC.checkConnectivity == 1
       for r1 = 1 : 2*nL+1
        for r2 = 1 : 2*nL+1
         for r3 = 1 : 2*nL+1
          for m = 1 : N_i
...
if ORG_STRUC.dimension==0 %cluster
      for i = 1 : N_i
        if (nL == 0) & (colors(1,1,1,i) ~= c1)
         ...
         if (colors(1,1,1,i) ~= c1) & (c2 == 0)
           c2 = colors(1,1,1,i);
         elseif (colors(1,1,1,i) ~= c1) & (colors(1,1,1,i) ~= c2)
          connected = 0;
         end
        end
      end
   ...
   if connected | (it >= N_bonds) | (ORG_STRUC.checkConnectivity == 0)
