...
for indic = 1:ORG_STRUC.numParallelCalcs
    ...
    if ~isempty (whichInd)
       ...
       if POP_STRUC.POPULATION(whichInd).JobID
           ...
           if doneOr
              if (POP_STRUC.POPULATION(whichInd).JobID == 0.01) | (POP_STRUC.POPULATION(whichInd).JobID == 0.02)   % reoptOld = 0, survived structure
                 POP_STRUC.POPULATION(whichInd).Step = length([ORG_STRUC.abinitioCode]) + 1;
              else%if %POP_STRUC.POPULATION(whichInd).JobID ~= 0.02   % no optimization
