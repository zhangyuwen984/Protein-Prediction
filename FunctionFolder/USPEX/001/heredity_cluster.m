function [numIons, offspring,potentialLattice,fracFrac,dimension,offset,fracLattice] = heredity_cluster(parent1,parent2,parentlat1,parentlat2, order1, order2, numIons1, numIons2, fracFrac)
...
%%%%%001mode%%%
%%numIons = ORG_STRUC.numIons; %000mode
...
for ind = 1 : length(numIons1)
  ionChange(ind) = sum(numIons1(1:ind));
end
ionCh1 = zeros(1,length(ionChange)+1);
ionCh1(2:end) = ionChange;

for ind = 1 : length(numIons2)
  ionChange(ind) = sum(numIons2(1:ind));
end
ionCh2 = zeros(1,length(ionChange)+1);
ionCh2(2:end) = ionChange;
%%%%%
...
for ind = 1:length(ORG_STRUC.atomType)
    ...
    numIons(ind) = ionCount(ind,1);  %%%001mode

    if ionCount(ind,1) < ORG_STRUC.numIons(1,ind)   %numIons(ind)     %%%001mode - binary clusters
        numIons(ind) = ORG_STRUC.numIons(1,ind);                      %%%001mode - binary clusters

        % calculate how many ions (of this type) are missing
        howmany = ORG_STRUC.numIons(1,ind) - ionCount(ind,1);		%%001mode - binary clusters
        ...
        if suppleIons_1 > 0
            ...
            %%%%disp - test
            %disp([num2str(ionCh1(ind)+1),' ',num2str(ionCh1(ind+1)),' ',num2str(dimension)]);
            %disp(num2str(parent2(ionCh1(ind)+1:ionCh1(ind+1),dimension)));
            %disp(num2str(fracFrac));
            %%%%%%%%%%%%%%%
        ...
        if suppleIons_2 > 0
            ...
            %%%%disp - test
            %disp([num2str(ionCh1(ind)+1),' ',num2str(ionCh1(ind+1)),' ',num2str(dimension)]);
            %disp(num2str(parent1(ionCh1(ind)+1:ionCh1(ind+1),dimension)));
            %disp(num2str(fracFrac));
            %%%%%%%%%%%%%%%
    ...
    elseif  ionCount(ind,1)>ORG_STRUC.numIons(2,ind)         %%%001mode - binary clusters
        numIons(ind) = ORG_STRUC.numIons(2,ind);             %%%001mode - binary clusters

        howmany = ionCount(ind,1) - ORG_STRUC.numIons(2,ind);  %%001mode - binary clusters
