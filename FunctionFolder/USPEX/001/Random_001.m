...
if size(ORG_STRUC.numIons,2) == 1      % one-component clusters
  numIons = ORG_STRUC.numIons(1,1) + round((ORG_STRUC.numIons(2,1) - ORG_STRUC.numIons(1,1)) * rand(1));
elseif size(ORG_STRUC.numIons,2) == 2  % two-component clusters
  numIons(1) = ORG_STRUC.numIons(1,1) + round((ORG_STRUC.numIons(2,1) - ORG_STRUC.numIons(1,1)) * rand(1));
  numIons(2) = ORG_STRUC.numIons(1,2) + round((ORG_STRUC.numIons(2,2) - ORG_STRUC.numIons(1,2)) * rand(1));
end
  ...
  [candidate, lat, errorS] = symope_001(nsym, numIons, lat, minDistMatrice*sym_coef);
