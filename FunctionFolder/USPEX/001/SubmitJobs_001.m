function SubmitJobs_001()
% One bug detected, we should do nothing for the kept structures
...
for indic = 1:ORG_STRUC.numParallelCalcs
    ...
    if DO_NOW
        ...
        elseif ORG_STRUC.abinitioCode(Step) == 0   % no optimization at all! (used in order optimization)
          ...
          [lat, candidate] = reduce_Cluster(POP_STRUC.POPULATION(DO_NOW).LATTICE, POP_STRUC.POPULATION(DO_NOW).COORDINATES);
