function [numIons, numBlocks, offspring, potentialLattice, fracFrac, dimension, offset, fracLattice] = heredity_final_var(par_one, par_two)
...
% in this way the most extreme difference is 1:3
 ...
 parent1 = POOL_STRUC.POPULATION(par_one).COORDINATES;
 lat1 = POOL_STRUC.POPULATION(par_one).LATTICE;
 order1 = POOL_STRUC.POPULATION(par_one).order;
 numIons1 = POOL_STRUC.POPULATION(par_one).numIons;
 numBlocks1 = POOL_STRUC.POPULATION(par_one).numBlocks;

 parent2 = POOL_STRUC.POPULATION(par_two).COORDINATES;
 lat2 = POOL_STRUC.POPULATION(par_two).LATTICE;
 order2 = POOL_STRUC.POPULATION(par_two).order;
 numIons2 = POOL_STRUC.POPULATION(par_two).numIons;
 numBlocks2 = POOL_STRUC.POPULATION(par_two).numBlocks;
if size(ORG_STRUC.numIons, 1) == 1 %for single block, always make the parents have same atoms before the marriage.
   if numBlocks1 > numBlocks2
      [lat2, parent2, order2, numIons2] = supercell_301(lat2, parent2, order2, numIons2, round(numBlocks2), round(numBlocks1));
       numBlocks2 = numBlocks1;
   elseif numBlocks1 < numBlocks2
      [lat1, parent1, order1, numIons1] = supercell_301(lat1, parent1, order1, numIons1, round(numBlocks1), round(numBlocks2));
       numBlocks1 = numBlocks2;
   end
end
 maxBlocks = numBlocks1 + numBlocks2;
...
if ~ORG_STRUC.constLattice
    ...
    if size(ORG_STRUC.numIons,1) == 2
       sameComp = (sameComposition(ORG_STRUC.numIons(1,:),ORG_STRUC.numIons(2,:)) == 0);
    else
       sameComp = 0;
    end

    if (ORG_STRUC.optType == 1) & sameComp
	    ...
	    latVol = 0; %best volume estimation based on numIons
        for it = 1 : length(ORG_STRUC.latVolume)
            latVol = latVol + numIons(it)*ORG_STRUC.latVolume(it);
        end
